...
class MOSDFailure : public PaxosServiceMessage {
 ...
 public:
  ...
  __u8 is_failed;
  epoch_t       epoch;
  utime_t failed_since;  // known to be failed since at least this long

  MOSDFailure() : PaxosServiceMessage(MSG_OSD_FAILURE, 0, HEAD_VERSION) { }
  MOSDFailure(const uuid_d &fs, entity_inst_t f, utime_t s, epoch_t e)
    : PaxosServiceMessage(MSG_OSD_FAILURE, e, HEAD_VERSION),
      fsid(fs), target_osd(f), is_failed(true), epoch(e), failed_since(s) { }
private:
  ~MOSDFailure() {}
...
public: 
  ...
  void decode_payload() {
    ...
    if (header.version >= 2)
      ...
      is_failed = true;
    if (header.version >= 3)
      ::decode(failed_since, p);
    else
      failed_since = utime_t();
  }
  void encode_payload(uint64_t features) {
    ...
    ::encode(failed_since, payload);
  }

  const char *get_type_name() const { return "osd_failure"; }
  void print(ostream& out) const {
    out << "osd_failure(" << target_osd << " since " << failed_since << " e" << epoch << " v" << version << ")";
  }
};
